...
impl Default for MoteChainConfig {
    fn default() -> Self {
        Self {
            max_btl: MAX_BTL,
...
mod tests {
    ...
    fn defaults_match_constants() {
        let config = MoteChainConfig::default();
    ...
    fn validate_zero_max_btl_rejected() {
        let mut config = MoteChainConfig::default();
        config.max_btl = 0;
    ...
    fn deserialize_partial_config_uses_defaults() {
        ...
        let config: MoteChainConfig = serde_json::from_str(json).expect("valid json");
